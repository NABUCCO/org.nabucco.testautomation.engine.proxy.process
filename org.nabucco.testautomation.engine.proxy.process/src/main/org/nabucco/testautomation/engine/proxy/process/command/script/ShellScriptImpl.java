	...
	public ActionResponse execute(TestContext context,
		...
		try {
			...
			case EXECUTE:
				...
				break;
			default:
				result.setErrorMessage("Unsupported ProcessActionType for ShellScript: "
						+ actionType);
			...
			PropertyList returnProperties = command.execute(metadata,
					propertyList);
			...
			String errorMessage = "Could not execute ShellScript-command. Cause: "
					+ ex.getMessage();
			...
			logger.fatal(ex);
			result.setErrorMessage("Could not execute ShellScript-command. Cause: "
					+ ex.toString());
			result.setActionStatus(ActionStatusType.FAILED);
			return result;
		} finally {
