 ...
 */
public class MakeDirCommand extends AbstractSFTPCommand implements SFTPCommand {
     ...
     *            the SFTP client
     */
    protected MakeDirCommand(SFTPClient sftpClient) {
        super(sftpClient);
    }
    ...
    protected PropertyList executeCallback(Metadata metadata, PropertyList properties) throws SFTPException {
        ...
        if (super.isConnected()) {
            String name = this.getPathname(metadata, properties);

            super.info("Creating directory: " + name);

            super.start();
            super.getSFTPClient().mkdir(name);
            super.stop();

            return null;
        }

        throw new SFTPException("Cannot create directory. Cause: Not connected");
    }
     ...
     * 
     * @return the path to make the directory to
     * 
