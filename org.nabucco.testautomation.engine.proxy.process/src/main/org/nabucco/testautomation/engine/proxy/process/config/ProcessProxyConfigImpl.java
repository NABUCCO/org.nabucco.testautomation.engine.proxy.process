...
import org.nabucco.testautomation.engine.proxy.process.config.ProcessProxyConfigurationType;
 ...
 * 
 */
public class ProcessProxyConfigImpl extends AbstractProxyEngineConfiguration
		implements ProcessProxyConfiguration {

	private static final NBCTestLogger logger = NBCTestLoggingFactory
			.getInstance().getLogger(ProcessProxyConfigImpl.class);
     ...
     */
	public ProcessProxyConfigImpl(ClassLoader classloader,
	...
	public int getFtpServerPort() {
		try {
			...
			logger.warning(e, "Cannot parse Property name='" + ProcessProxyConfigurationType.FTP_SERVER_PORT.getKey()
					+ "'. Using default FTP port:  " + DEFAULT_FTP_PORT);
     ...
     */
	@Override
	public InetAddress getBatchServerHost()
	...
	private InetAddress extractInetAddress(String assertHost)
			throws NBCTestConfigurationException {
		...
		try {
			...
			throw new NBCTestConfigurationException(
					"Configured Host is not valid: '" + assertHost + "'");
		}
	}
